...
func TestAccAzureRMLinuxVirtualMachineScaleSet_otherDedicatedHostGroup(t *testing.T) {
    data := acceptance.BuildTestData(t, "azurerm_linux_virtual_machine_scale_set", "test")
    ...
    resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMLinuxVirtualMachineScaleSet_otherDedicatedHostGroup(data),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMLinuxVirtualMachineScaleSetExists(data.ResourceName),
				),
			},
...
func testAccAzureRMLinuxVirtualMachineScaleSet_otherDedicatedHostGroup(data acceptance.TestData) string {
	template := testAccAzureRMLinuxVirtualMachineScaleSet_template(data)
	return fmt.Sprintf(`
%s
